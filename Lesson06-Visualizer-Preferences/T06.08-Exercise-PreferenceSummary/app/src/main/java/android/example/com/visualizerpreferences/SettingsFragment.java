...
public class SettingsFragment extends PreferenceFragmentCompat implements OnSharedPreferenceChangeListener {
    ...
    public void onCreatePreferences(Bundle bundle, String s) {
        ...
        int total = preferenceScreen.getPreferenceCount();
        for (int i = 0; i < total; i++) {
            Preference preference = preferenceScreen.getPreference(i);
            if (!(preference instanceof CheckBoxPreference)) {
                String getValue = sharedPreferences.getString(preference.getKey(), "");
                setPreferenceSummary(preference, getValue);
    ...
    private void setPreferenceSummary(Preference preference, String value) {
        if (preference instanceof ListPreference) {
            ...
            int indexVal = listPreference.findIndexOfValue(value);
            if (indexVal >= 0) {
                listPreference.setSummary(listPreference.getEntries()[indexVal]);
